...
* How do we write about data?
  ...
  * _Y_ generally represents outcomes, _X_ for covariates
* **Randomness**
  * _X_ & _Y_ as "random variables" 
  * a variable is random b/c it is an incompletely measured variable 
    * or it's a random draw from a population 
  * _X = x_ → fixing _X_ (random quantity _X_) to be observed/fixed _x_ 
...
* **Distributions**
  * _Pr_ = probability (b/w 0 and 1, must add up to 1)
  * our coin flip probability written as:
    * _Pr(X = 1) = 0.5 and Pr(X = 0) = 0.5_
...
* parameters 
  ...
  * usually represented w/ Greek letters 
    * _N(μ,σ)_ or _Poisson(λ)_ 
  * e.g., _X ~ N(μ,σ)_
    * translation: X has the N(μ,σ) distribution 
* 3 most important parameters 
  * _E[X]_ → expectation of X (measures the "center"; usually mean?) 
  * _Var[X]_ → variance of X 
  * _SD[X]_ → standard deviation 
* **Conditioning**
  * _X|μ_ → X is a random variable with fixed parameter μ 
...
* example distribution : **Uniform**
  ...
  * _U(α, β)_
    * _α, β_ = boundaries of which values we consider to be equally likely
  * X ~ U(0, 1) 
* **Changing parameters**
  * Normal distribution:
    * _X ~ N(0, 1) , E[X] = μ = 0, Var[X] = σ² = 1_
      * vs. 
    * _X ~ N(0, 5) , E[X] = μ = 0, Var[X] = σ² = 25_
      * (a fatter "bell curve" function)
      * vs. 
    * _X ~ N(5, 1) , E[X] = μ = 5, Var[X] = σ² = 1_
      * (skewed)
  * Binomial:
    * _X ~ Bin(10, 0.5), E[X] = n × p = 5, Var[X] = n × p (1 - p) = 2.5_
      * vs.
    * _X ~ Bin(10, 0.8), E[X] = n × p = 8, Var[X] = n × p (1 - p) = 1.6_
      * (skewed)
  * CONDITION:
    * Suppose : _Y ~ N(X, 1) and X ~ N(0, 1), then distribution of Y|X = 5_
...
* Style guides are out there for R:
  * check it: [Google's R style guide](http://google-styleguide.googlecode.com/svn/trunk/google-r-style.html)
...
* 3 categories:
  ...
  * sampling
* "r functions" 
  * to generate data → e.g., `coin.flips <- rbinom(10, size=10, prob=0.5)`
* "d functions"
...
* what kind of question are you trying to answer? try categories:
  ...
  * **predictive** → goal : use the data on some objects to predict values for another object
    ...
    * case study : how Target figured out how a girl was pregnant before her father did (source Forbes.com)
